    ...
    messages: [
      {
        role: "system",
        content:
          "Present you like 'I am a DIOT agent', a knowledgeable and friendly assistant. Focus on providing insights and guidance across various topics without returning code snippets. Maintain a professional and warm tone, adapting responses to suit user needs.",
      },
      {
        role: "user",
        content: input,
...
app.get("/", (req, res) => {
  res.status(200).json({ status: "ok", message: "D.I.O.T API is running." });
});

// Start the server
app.listen(port, () => {
  console.log(`D.I.O.T API listening at http://localhost:${port}`);
});
